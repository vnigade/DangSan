...
: ${INSTANCES="default baselinesafestack metaalloc baseline"}
#: ${INSTANCES="metaalloc"}
: ${JOBS=16}
: ${LLVMREV:=251286}
: ${REPLACEGLIBC:=0}
...
EXTRA_LDFLAGS_BASELINE="-L$PATHAUTOSRC/gperftools/.libs -ltcmalloc"

EXTRA_LDFLAGS_BASELINE_SAFESTACK="-flto -fsanitize=safe-stack -Wl,-plugin-opt=-load=/home/vne500/Metalloc/llvm-plugins/libplugins.so -Wl,-plugin-opt=-byvalhandler -L/home/vne500/Metalloc/staticlib -Wl,-whole-archive -l:libmetadata.a -Wl,-no-whole-archive /home/vne500/Metalloc/metapagetable/obj/metapagetable.o -L/home/vne500/Metalloc/autosetup.dir/src/gperftools/.libs -ltcmalloc"
...
for instance in $INSTANCES; do
	...
	[ "$instance" = baseline ] && echo "LLVMGOLD_LDFLAGS_EXTRA=$EXTRA_LDFLAGS_BASELINE" > common.overrides.baseline.inc
        [ "$instance" = baselinesafestack ] && echo "LLVMGOLD_LDFLAGS_EXTRA=$EXTRA_LDFLAGS_BASELINE_SAFESTACK" > common.overrides.baseline.inc
...
for instance in $INSTANCES; do
	...
	touch init.done
	PATH="$prefix/bin:$PATH"  C="$BENCHMARKS" run "$PATHAUTOPREFIX/bin/bash" ./configure.llvm
	#PATH="$prefix/bin:$PATH" C=$1 run "$PATHAUTOPREFIX/bin/bash" ./configure.llvm
	# create config files for baseline and instrumented SPEC
	cp "$SPEC_LLVM_ROOT/config/config.llvm.cfg" "$SPEC_LLVM_ROOT/config/config.llvm-baseline.cfg"
        if [ "$instance" = baselinesafestack -o "$instance" = metaalloc ]; then
            sed -i "s|^EXTRA_CFLAGS=|EXTRA_CXXFLAGS=-fsanitize=safe-stack\nEXTRA_CFLAGS=-fsanitize=safe-stack |" "$SPEC_LLVM_ROOT/config/config.llvm-baseline.cfg"
        fi

        if [ "$DANG_DEBUG" -ne 1 ]; then
            sed -i "s|^COPTIMIZE=|CXXOPTIMIZE=-flto -O3 -fno-strict-aliasing\nCOPTIMIZE = -flto -O3 -fno-strict-aliasing -std=gnu89 |" "$SPEC_LLVM_ROOT/config/config.llvm-baseline.cfg"
        else
            sed -i "s|^COPTIMIZE=|CXXOPTIMIZE=-g3 -fno-strict-aliasing\nCOPTIMIZE   = -g3 -fno-strict-aliasing -std=gnu89 |" "$SPEC_LLVM_ROOT/config/config.llvm-baseline.cfg"
        fi

	cp "$SPEC_LLVM_ROOT/config/config.llvm-baseline.cfg" "$SPEC_LLVM_ROOT/config/config.llvm-metaalloc.cfg"
        if [ "$instance" = baseline ]; then
            sed -i "s|^EXTRA_LDFLAGS=|EXTRA_LDFLAGS=$EXTRA_LDFLAGS_BASELINE |" "$SPEC_LLVM_ROOT/config/config.llvm-baseline.cfg"
        fi 
        
        if [ "$instance" = baselinesafestack ]; then
            sed -i "s|^EXTRA_LDFLAGS=|EXTRA_LDFLAGS=$EXTRA_LDFLAGS_BASELINE_SAFESTACK |" "$SPEC_LLVM_ROOT/config/config.llvm-baseline.cfg"
        fi
            
	sed -i "s|^EXTRA_LDFLAGS=|EXTRA_LDFLAGS=$EXTRA_LDFLAGS_METAALLOC |" "$SPEC_LLVM_ROOT/config/config.llvm-metaalloc.cfg"
	# select config file for instance
	CONFIGNAME=config.llvm.cfg
	[ "$instance" = baseline ] && CONFIGNAME=config.llvm-baseline.cfg
        if [ "$instance" = baselinesafestack ]; then
            mv "$SPEC_LLVM_ROOT/config/config.llvm-baseline.cfg" "$SPEC_LLVM_ROOT/config/config.llvm-baselinesafestack.cfg"
            CONFIGNAME=config.llvm-baselinesafestack.cfg
        fi

	[ "$instance" = metaalloc ] && CONFIGNAME=config.llvm-metaalloc.cfg
	# this code is from the remaining part of configure.llvm
	source "$SPEC_LLVM_ROOT/shrc"
	for d in $( find . -wholename \*benchspec/CPU2006/[0-9]\*/Docs -exec dirname {} \; )
	do
		(cd $d; mkdir -p run; rm -rf build; mkdir -p build)
	done
        
        #if [ "$DANG_DEBUG" = 1 ]; then
            #exit
        #fi

	PATH="$prefix/bin:$PATH" CONFIGNAME="$CONFIGNAME" LD_LIBRARY_PATH="$PATHAUTOPREFIX/lib:$LD_LIBRARY_PATH" CLEAN=1 C="$BENCHMARKS" run "$PATHAUTOPREFIX/bin/bash" ./relink.llvm
	...
	(
		...
		[ "$instance" = baseline -o "$instance" = baselinesafestack ] && echo "export METALLOC_PATH=\"$PATHAUTOSRC/gperftools\""
		...
		[ "$instance" = baseline -o "$instance" = baselinesafestack ] && echo "export LD_LIBRARY_PATH=\"$PATHAUTOPREFIXBASE/lib:\$LD_LIBRARY_PATH\""
	...
	(
		...
		echo "#BENCH_MEM=\"1\" ./clientctl bench"
