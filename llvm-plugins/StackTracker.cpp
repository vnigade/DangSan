...
struct StackTracker : public FunctionPass {
    ...
    virtual bool runOnFunction(Function &F) {
        ...
        /* DangSan: Get all return instructions.
         * Comment it out if Stack objects has to be tracked.
         * TODO: Make this compile time option.
        std::vector<TerminatorInst*> returnInstructions;
        for (Function::iterator BI = F.getBasicBlockList().begin(); BI != F.getBasicBlockList().end();
                    ++BI) {
            TerminatorInst *term = BI->getTerminator();
            if (isa<ReturnInst>(term) || isa<UnreachableInst>(term))
                returnInstructions.push_back(term);
        }
        */ /* END: DangSan */
        ...
        for (auto AI : interestingAllocas) {
            ...
            if (!DeepMetadata) {
                ...
                /* DangSan : 
                 * Add DangSan init function for each metadata
                 * TODO: Make compile time option.
                std::vector<Value *> callArgs;
                callArgs.push_back(ptr);
                callArgs.push_back(size);
                B.CreateCall(DangSanInitFunc, callArgs);
                // Loop through all return instructions and insert free function
                for (auto RetInst : returnInstructions) {
                    if (!DT->dominates(dyn_cast<Instruction>(ptr), RetInst))
                        continue;
                    Instruction *insertBefInst = RetInst;
                    IRBuilder<>  B1(insertBefInst);
                    B1.CreateCall(DangSanFreeFunc, callArgs);
                }               
                */  /* END: DangSan */

                /* For each return instruction add DangSan free function */
                if (FixedCompression || metaEntry == NULL) {
                    /* DangSan:
                     * Stack objects are not tracked. We do not need this.
                     */
